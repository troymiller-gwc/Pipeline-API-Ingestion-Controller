    ...
    steps: [
      {
        ...
        dependsOn: ["4.4", "2.4"],
        tasks: [
          { label: "API endpoint to sync schedule_cron → Cloud Scheduler jobs", done: true },
          { label: "Scheduler trigger → creates extraction_run → launches Cloud Run Job", done: true },
          { label: "Handle schedule enable/disable when endpoint is_active changes", done: true },
        ],
      ...
      },
      {
        ...
        tasks: [
          { label: "Dockerfile for API server (Express)", done: true },
          { label: "Dockerfile for frontend (Vite build → nginx/static serve)", done: true },
          { label: "Dockerfile for Cloud Run Job (extraction engine)", done: true },
          { label: "Multi-stage builds for minimal image sizes", done: true },
          { label: "Environment variable configuration for each container", done: true },
        ],
      ...
      },
      {
        ...
        dependsOn: ["5.2"],
        tasks: [
          { label: "CI workflow: lint, typecheck, test on PR", done: true },
          { label: "CD workflow: build Docker images on merge to main", done: true },
          { label: "Push images to Google Artifact Registry", done: true },
          { label: "Deploy API + Frontend to Cloud Run", done: true },
          { label: "Deploy extraction job to Cloud Run Jobs", done: true },
          { label: "Update Cloud Scheduler jobs", done: true },
          { label: "GCP service account and Workload Identity Federation setup", done: true },
        ],
      ...
      },
      {
        ...
        status: "complete",
        tasks: [
          { label: "Cloud SQL PostgreSQL production instance (or Terraform/Pulumi IaC)", done: true },
          { label: "Run migrations against production Cloud SQL", done: true },
          { label: "Secret Manager secrets for API credentials (NICE CXone, etc.)", done: true },
          { label: "IAM service accounts with least-privilege roles", done: true },
          { label: "VPC/networking for Cloud SQL private access from Cloud Run", done: true },
          { label: "Workload Identity Federation for GitHub Actions → GCP", done: true },
        ],
