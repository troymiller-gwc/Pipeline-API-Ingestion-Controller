    ...
    steps: [
      {
        ...
        status: "not-started",
        tasks: [
          { label: "Set up migration tool (drizzle-orm or raw SQL migrations)", done: false },
          { label: "Create control.source_system table with PK, constraints", done: false },
          { label: "Create control.endpoint_definition table with FK to source_system", done: false },
          { label: "Create control.endpoint_parameter table with FK and UNIQUE constraint", done: false },
          { label: "Create ops.extraction_run table with self-referencing parent_run_id FK", done: false },
          { label: "Create ops.extraction_event table with FK to extraction_run", done: false },
          { label: "Add all indexes (idx_run_endpoint_status, idx_run_created, idx_event_run, etc.)", done: false },
          { label: "Seed NICE CXone source_system and get_contacts endpoint_definition", done: false },
        ],
      ...
      },
      {
        ...
        status: "not-started",
        tasks: [
          { label: "Define enums: PaginationStrategy, IncrementalStrategy, RunType, RunStatus, etc.", done: false },
          { label: "Define Zod schemas for source_system CRUD", done: false },
          { label: "Define Zod schemas for endpoint_definition CRUD", done: false },
          { label: "Define Zod schemas for endpoint_parameter CRUD", done: false },
          { label: "Define Zod schemas for extraction_run (create, status update)", done: false },
          { label: "Define Zod schemas for extraction_event", done: false },
          { label: "Export inferred TypeScript types from all schemas", done: false },
        ],
      ...
      },
      {
        ...
        status: "not-started",
        tasks: [
          { label: "Create raw dataset in BigQuery", done: false },
          { label: "Create raw.api_payload table with all columns", done: false },
          { label: "Configure partitioning by DATE(ingested_ts)", done: false },
          { label: "Configure clustering by source_system_id, endpoint_id, run_id", done: false },
          { label: "Set up BigQuery client library and connection config", done: false },
        ],
        outputs: ["BigQuery raw.api_payload table", "BigQuery client setup"],
      },
      {
        id: "1.4",
        ...
        dependsOn: ["1.1"],
        tasks: [
          { label: "Configure PostgreSQL connection pool (pg or drizzle)", done: false },
          { label: "Set up environment-based connection config (local vs Cloud SQL)", done: false },
          { label: "Create typed query helpers for each table", done: false },
          { label: "Add transaction support wrapper", done: false },
        ],
    ...
    steps: [
      {
        ...
        dependsOn: ["1.2", "1.4"],
        tasks: [
          { label: "GET /api/source-systems — list all source systems", done: false },
          { label: "GET /api/source-systems/:id — get single source system", done: false },
          { label: "POST /api/source-systems — create source system", done: false },
          { label: "PUT /api/source-systems/:id — update source system", done: false },
          { label: "DELETE /api/source-systems/:id — soft delete (set is_active=false)", done: false },
          { label: "Input validation with Zod schemas", done: false },
          { label: "Error handling middleware", done: false },
        ],
      ...
      },
      {
        ...
        dependsOn: ["2.1"],
        tasks: [
          { label: "GET /api/endpoints — list endpoints (filterable by source_system_id)", done: false },
          { label: "GET /api/endpoints/:id — get single endpoint with parameters", done: false },
          { label: "POST /api/endpoints — create endpoint definition", done: false },
          { label: "PUT /api/endpoints/:id — update endpoint definition", done: false },
          { label: "DELETE /api/endpoints/:id — soft delete", done: false },
          { label: "Validate pagination_strategy and incremental_strategy enum values", done: false },
        ],
      ...
      },
      {
        ...
        dependsOn: ["2.2"],
        tasks: [
          { label: "GET /api/endpoints/:id/parameters — list parameters for endpoint", done: false },
          { label: "POST /api/endpoints/:id/parameters — create parameter", done: false },
          { label: "PUT /api/parameters/:id — update parameter", done: false },
          { label: "DELETE /api/parameters/:id — delete parameter", done: false },
          { label: "Enforce UNIQUE(endpoint_id, parameter_name) constraint", done: false },
          { label: "Support display_order for form field ordering", done: false },
        ],
      ...
      },
      {
        ...
        dependsOn: ["2.2"],
        tasks: [
          { label: "POST /api/runs — create and trigger a new extraction run", done: false },
          { label: "GET /api/runs — list runs with filtering and pagination", done: false },
          { label: "GET /api/runs/:id — get run detail with events", done: false },
          { label: "Implement concurrency guard (SELECT...FOR UPDATE, reject if active run exists)", done: false },
          { label: "Request builder: resolve endpoint_definition + parameters into full HTTP request", done: false },
          { label: "GET /api/endpoints/:id/preview — build request preview without executing", done: false },
        ],
      ...
      },
      {
        ...
        dependsOn: ["2.4"],
        tasks: [
          { label: "GET /api/runs/:id/events — list events for a run", done: false },
          { label: "GET /api/runs/:id/pages — list page-level status from BigQuery", done: false },
          { label: "PATCH /api/runs/:id/cancel — cancel a queued/running run", done: false },
          { label: "POST /api/runs/:id/replay — create replay run for failed pages", done: false },
        ],
    ...
    steps: [
      {
        ...
        dependsOn: ["2.1"],
        tasks: [
          { label: "Create app layout with sidebar navigation", done: false },
          { label: "Set up client-side routing (React Router or wouter)", done: false },
          { label: "Configure API client with base URL and error handling", done: false },
          { label: "Add loading states and error boundary components", done: false },
        ],
      ...
      },
      {
        ...
        dependsOn: ["3.1", "2.1"],
        tasks: [
          { label: "Source system list page with status indicators", done: false },
          { label: "Create/edit source system form (name, base_url, auth_type, secret ref)", done: false },
          { label: "Delete confirmation dialog", done: false },
          { label: "Toast notifications for CRUD operations", done: false },
        ],
      ...
      },
      {
        ...
        dependsOn: ["3.1", "2.2"],
        tasks: [
          { label: "Endpoint list page filtered by source system", done: false },
          { label: "Create/edit endpoint form with all fields", done: false },
          { label: "Pagination strategy selector with dynamic config fields", done: false },
          { label: "Incremental strategy selector with config fields", done: false },
          { label: "Parameter management sub-section (inline CRUD, drag-to-reorder)", done: false },
        ],
      ...
      },
      {
        ...
        dependsOn: ["3.3", "2.4"],
        tasks: [
          { label: "Dynamic form rendered from endpoint_parameter metadata", done: false },
          { label: "Field types: text, number, date, dropdown (from allowed_values_json)", done: false },
          { label: "Request preview panel showing the full HTTP request before execution", done: false },
          { label: "Confirm and trigger button with loading state", done: false },
          { label: "Redirect to run detail page after trigger", done: false },
        ],
      ...
      },
      {
        ...
        dependsOn: ["3.1", "2.5"],
        tasks: [
          { label: "Run history list with status badges, filtering, and sorting", done: false },
          { label: "Run detail page with metadata, timing, and counts", done: false },
          { label: "Events timeline with severity-colored entries", done: false },
          { label: "Page-level status breakdown (succeeded/failed/replayed counts)", done: false },
          { label: "Replay button for failed/partial runs", done: false },
          { label: "Cancel button for queued/running runs", done: false },
        ],
    ...
    steps: [
      {
        ...
        status: "not-started",
        tasks: [
          { label: "Secret Manager client for credential resolution at startup", done: false },
          { label: "API_KEY auth handler (static key in Authorization header)", done: false },
          { label: "OAUTH2_CLIENT_CREDENTIALS handler with token acquisition", done: false },
          { label: "In-memory token cache with proactive refresh (<10% TTL remaining)", done: false },
          { label: "Mid-run token expiration re-acquisition", done: false },
          { label: "AUTH_REFRESHED event logging to extraction_event", done: false },
          { label: "Fatal startup error if Secret Manager unavailable", done: false },
        ],
      ...
      },
      {
        ...
        dependsOn: ["4.1"],
        tasks: [
          { label: "NONE strategy — single request, no pagination", done: false },
          { label: "PAGE_NUMBER strategy — page=1, page=2, ...", done: false },
          { label: "OFFSET_LIMIT strategy — skip/top or offset/limit", done: false },
          { label: "NEXT_TOKEN strategy — cursor/token-based from response", done: false },
          { label: "Token bucket rate limiter from rate_limit_config_json", done: false },
          { label: "Checkpoint saving after each page", done: false },
          { label: "Page-level event logging (PAGE_REQUESTED, PAGE_RECEIVED)", done: false },
        ],
      ...
      },
      {
        ...
        dependsOn: ["1.3"],
        tasks: [
          { label: "BigQuery client setup for raw.api_payload writes", done: false },
          { label: "Construct api_payload row from HTTP response", done: false },
          { label: "Compute payload_hash (SHA-256) for deduplication", done: false },
          { label: "Batch inserts for efficiency", done: false },
          { label: "Error handling for BigQuery write failures", done: false },
        ],
      ...
      },
      {
        ...
        tasks: [
          { label: "Job entry point: receive run_id, load run config from PostgreSQL", done: false },
          { label: "Update run status: QUEUED → RUNNING → SUCCEEDED/FAILED/PARTIAL_SUCCESS", done: false },
          { label: "Orchestrate: auth → paginate → write payload → log events → checkpoint", done: false },
          { label: "Failure handling: categorize errors, update counts, save error_summary", done: false },
          { label: "Replay mode: query failed pages from original run, retry only those offsets", done: false },
          { label: "Mark replayed pages as REPLAYED in BigQuery", done: false },
        ],
